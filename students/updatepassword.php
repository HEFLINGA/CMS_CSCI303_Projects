...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($error_exists == 1) {
        ...
        $sql = "UPDATE students SET pwd = :pwd WHERE ID = :ID";
...
if($show_form == 1) {
    ...
    <div class="form_small">
        <p class="informative">*Once the form is submitted, you will be logged out an be required to re-log in with your new password.</p>
        <form name="updatepwd" id="updatepwd" method="post" action="<?php echo $currentFile; ?>">
            ...
            <?php if (!empty($error_pwd1)) { echo "<span class='error'>$error_pwd1</span><br>"; } ?>
            <input type="password" name="pwd1" id="pwd1"
                   size="40" placeholder="Enter Password">
            <br>
            <label class="label_text" for="pwd2">Confirm Password:</label><br>
            <?php if (!empty($error_pwd2)) { echo "<span class='error'>$error_pwd2</span><br>"; } ?>
